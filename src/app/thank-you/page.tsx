...
const ThankYouPage = async ({ searchParams }: PageProps) => {
  ...
  // This page needs to be very well protected, to only allow authorized users to download content that they own.

  const { user } = await getServerSideUser(nextCookies);

  // Getting the actual order that this page is for
  const payload = await getPayloadClient();

  const { docs: orders } = await payload.find({
    collection: "orders",
    depth: 2, // Instead of just giving id and joining those together, for example for the user it will give us the actual user.
    where: {
      id: {
        equals: orderId,
      },
  ...
  // The user is logged in, also, the user trying to access this page is the same one that bought this order
  const orderUserId =
